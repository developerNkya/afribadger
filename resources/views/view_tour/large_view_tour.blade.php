...
<script>
 document.addEventListener("DOMContentLoaded", function () {
  // Select all image elements that trigger the display of the itinerary
  const imageViews = document.querySelectorAll('.imageview_d207cb065864');

  // Select all button-expandable elements for hiding the itinerary
  const expandableButtons = document.querySelectorAll('.button-expandable');

  // Iterate over each image and its corresponding expandable button
  imageViews.forEach((imageView, index) => {
    // Find the closest parent container of the clicked image that contains the itinerary
    const parentContainer = imageView.closest('.wp-block-group.container_c9f1b11a1b51');
    
    // Find the .opened-itenary inside this parent container
    const itinerary = parentContainer ? parentContainer.querySelector('.opened-itenary') : null;

    // If itinerary is not found, log an error
    if (!itinerary) {
      console.error('Itinerary not found for the current image view.');
      return;
    }

    // Find the expandable button that corresponds to the current itinerary
    const expandableButton = expandableButtons[index];

    // Show the itinerary when the image is clicked
    imageView.addEventListener('click', function () {
      itinerary.classList.add('show'); // Show the itinerary
      expandableButton.src = '../../assets/images/button-expandable.png'; // Change to minus icon
    });

    // Hide the itinerary when the expandable (plus) button is clicked
    expandableButton.addEventListener('click', function () {
      itinerary.classList.remove('show'); // Hide the itinerary
      expandableButton.src = '../../assets/images/button-expandable.png'; // Change back to plus icon
    });
  });
});
